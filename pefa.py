...
def get_pdf_file_path(link_to_content):
    parts = urlsplit(link_to_content)
    node_number = parts.path.split('/')[-1]
    return f'data/pdfs/{node_number}.pdf'

def download_pdf(link_to_content):
    download_file_path = get_pdf_file_path(link_to_content)
...
meta_df_english = meta_df_to_process[meta_df_to_process.Language == 'English']
for link_to_content in meta_df_english['Link to Content']:
    download_pdf(link_to_content)

# For each pdf report locate the table then extract Calculation sheets for PI 1, PI 2, PI 3
keyword = "(?:Calculation(?:s)?|Data) (?:.* )?pi"
secondary_keywords = ['budg(?:et)?', '(?:actu(?:al)?)']
tertiary_keywords = ['data for year', 'deviation', 'administrative']
for report in glob.glob('data/pdfs/*.pdf'):
